    ...
    input:
        expand(
            ...
            other_species=genomes[
                (~genomes["Species"].isin(["Capra_hircus", "Bos_taurus"]))
...
rule last_train_distant_assembly_vs_reference:
    ...
    output:
        protected(
            "GENOMES/Sus_scrofa/Sscrofa11.1/last_train_distant/{other_species}/{other_assembly_name}/GCF_000003025.6_{other_assembly_accession}.train"
        ),
    shell:
...
rule minigraph_generate_graph:
    input:
        ...
        others=expand(
            "GENOMES/Sus_scrofa/{other_assembly_name}/ncbi/{other_assembly_accession}_{other_assembly_name}_genomic_masked_mtgenome.fna.gz",
            zip,
            other_assembly_name=genomes[
                (genomes["Species"] != "Sus_scrofa")
                & (genomes["Assembly Name"] != "Sscrofa11.1")
            ]["Assembly Name"],
            other_assembly_accession=genomes[
                (genomes["Species"] == "Sus_scrofa")
...
rule mumandco_test:
    ...
    output:
        directory(
            "ANALYSES/mumandco/Sus_scrofa/{other_assembly_name}/{other_assembly_accession}/"
        ),
    threads: 4
    shell:
