...
rule download_assembly_report_and_uncomment_header:
    output:
        temp(
            "{genomes_folder}/{species}/{assembly_name}/ncbi/{assembly_accession}_{assembly_name}_assembly_report_to_edit.txt"
        ),
    shell:
...
rule add_refseq_or_genbank_unique_column_to_report:
    ...
    run:
        ...
        tempreport["Sequence-ID"] = tempreport["RefSeq-Accn"].where(
            tempreport["RefSeq-Accn"] != "na", tempreport["GenBank-Accn"]
        )
        ...
        tempreport["Chromosome/Scaffold Name"] = tempreport["Assigned-Molecule"].where(
            tempreport["Assigned-Molecule"] != "na", tempreport["Sequence-ID"]
        )
